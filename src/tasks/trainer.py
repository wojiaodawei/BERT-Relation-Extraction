...
def train_and_fit(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        print(
            "Epoch finished, took %.2f seconds." % (time.time() - start_time)
        )
    ...
    plt.savefig(
        os.path.join(
            "./data/", "task_train_accuracy_vs_epoch_%d.png" % args.model_no
        )
    )
    ...
    plt.savefig(
        os.path.join("./data/", "task_test_f1_vs_epoch_%d.png" % args.model_no)
    )
