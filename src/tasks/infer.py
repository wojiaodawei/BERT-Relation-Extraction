    ...
    def get_all_sub_obj_pairs(self, sent):
        ...
        if (subject is not None) and (len(objs) > 0):
            for a, b in permutations([subject] + [obj for obj in objs], 2):
                ...
                pairs.append(
                    (
                        a_[0] if (len(a_) == 1) else a_,
                        b_[0] if (len(b_) == 1) else b_,
                    )
                )
...
class FewRel(object):
    ...
    def evaluate(self):
        ...
        print(
            "Results (%d samples): %.3f %%" % (counts, (hits / counts) * 100)
        )
