...
class PreTrainedTokenizer(object):
    """
    Base class for all tokenizers. Handle all the shared methods for
    tokenization and special tokens as well as methods
    downloading/caching/loading pretrained tokenizers as well as adding tokens
    to the vocabulary.
    ...
    Class attributes (overridden by derived classes):
        ...
        - ``vocab_files_names``: a python ``dict`` with, as keys, the ``__init__`` keyword name of each vocabulary file required by the models, and as associated values, the filename for saving the associated file (string).
        - ``pretrained_vocab_files_map``: a python ``dict of dict`` the high-level keys being the ``__init__`` keyword name of each vocabulary file required by the models, the low-level being the `short-cut-names` (string) of the pretrained models with, as associated values, the `url` (string) to the associated pretrained vocabulary file.
        - ``max_model_input_sizes``: a python ``dict`` with, as keys, the `short-cut-names` (string) of the pretrained models, and as associated values, the maximum length of the sequence inputs of this models, or None if the models has no maximum input size.
        - ``pretrained_init_configuration``: a python ``dict`` with, as keys, the `short-cut-names` (string) of the pretrained models, and as associated values, a dictionnary of specific arguments to pass to the ``__init__``method of the tokenizer class for this pretrained models when loading the tokenizer with the ``from_pretrained()`` method.
    ...
    Parameters:
        ...
        - ``cls_token``: (`Optional`) string: a classification token (e.g. to extract a summary of an input sequence leveraging self-attention along the full depth of the models). Will be associated to ``self.cls_token`` and ``self.cls_token_id``

        - ``mask_token``: (`Optional`) string: a masking token (e.g. when training a models with masked-language modeling). Will be associated to ``self.mask_token`` and ``self.mask_token_id``
    ...
    def sep_token(self):
        """
        Separation token (string).

        E.g. separate context and query in an input sequence. Log an
        error if used while not having been set.
        """
    ...
    def cls_token(self):
        ...
        E.g. to extract a summary of an input sequence leveraging self-
        attention along the full depth of the models. Log an error if
    ...
    def mask_token(self):
        ...
        E.g. when training a models with masked-language modeling. Log
    ...
    def additional_special_tokens(self):
        """
        All the additional special tokens you may want to use (list of
        strings).
    ...
    def additional_special_tokens_ids(self):
        """
        Ids of all the additional special tokens in the vocabulary (list of
        integers).
    ...
    def _from_pretrained(
        ...
        if pretrained_model_name_or_path in s3_models:
            ...
            logger.info(
                "Model name '{}' not found in models shortcut name list ({}). "
                "Assuming '{}' is a path, a models identifier, or url to a directory containing tokenizer files.".format(
                    pretrained_model_name_or_path,
                    ", ".join(s3_models),
                    pretrained_model_name_or_path,
                )
            ...
            if os.path.isfile(pretrained_model_name_or_path) or is_remote_url(
                ...
                if len(cls.vocab_files_names) > 1:
                    raise ValueError(
                        ...
                        "Use a models identifier or the path to a directory instead.".format(
                            cls.__name__
                        )
                    )
                logger.warning(
                ...
                # At this point pretrained_model_name_or_path is either a directory or a models identifier name
        ...
        try:
            ...
            if pretrained_model_name_or_path in s3_models:
                ...
                msg = (
                    "Model name '{}' was not found in tokenizers models name list ({}). "
        ...
        if all(
            ...
            raise EnvironmentError(
                "Model name '{}' was not found in tokenizers models name list ({}). "
                "We assumed '{}' was a path, a models identifier, or url to a directory containing vocabulary files "
                "named {} but couldn't find such vocabulary files at this path or url.".format(
                    pretrained_model_name_or_path,
                    ", ".join(s3_models),
                    pretrained_model_name_or_path,
        ...
        if pretrained_model_name_or_path in cls.max_model_input_sizes:
            # if we're using a pretrained models, ensure the tokenizer
        ...
        if added_tokens_file is not None:
            with open(
                added_tokens_file, encoding="utf-8"
            ) as added_tokens_handle:
    ...
    def save_pretrained(self, save_directory):
        ...
        if len(self.added_tokens_encoder) > 0:
            with open(added_tokens_file, "w", encoding="utf-8") as f:
                out_str = json.dumps(
                    self.added_tokens_encoder, ensure_ascii=False
                )
                f.write(out_str)
    ...
    def save_vocabulary(self, save_directory):
        """
        Save the tokenizer vocabulary to a directory. This method does *NOT*
    ...
    def add_tokens(self, new_tokens):
        ...
        Examples::
            ...
            # Let's see how to increase the vocabulary of Bert models and tokenizer
            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            models = BertModel.from_pretrained('bert-base-uncased')

            num_added_toks = tokenizer.add_tokens(['new_tok1', 'my_new-tok2'])
            print('We have added', num_added_toks, 'tokens')
            models.resize_token_embeddings(len(tokenizer))  # Notice: resize_token_embeddings expect to receive the full size of the new vocabulary, i.e. the length of the tokenizer.
        """
        if not new_tokens:
            return 0
    ...
    def add_special_tokens(self, special_tokens_dict):
        ...
        vocabulary, they are added to it (indexed starting from the last index
        of the current vocabulary).

        Using `add_special_tokens` will ensure your special tokens can be used in several ways:

        - special tokens are carefully handled by the tokenizer (they are never split)
        - you can easily refer to special tokens using tokenizer class attributes like `tokenizer.cls_token`. This makes it easy to develop models-agnostic training and fine-tuning scripts.
        ...
        Examples::
            ...
            models = GPT2Model.from_pretrained('gpt2')
    ...
    def tokenize(self, text, **kwargs):
        """
        Converts a string in a sequence of tokens (string), using the
        ...
        **kwargs: passed to the child `self.tokenize()` method
        ...
        def lowercase_text(t):
            ...
            return re.sub(
                pattern, lambda m: m.groups()[0] or m.groups()[1].lower(), t
            )
    ...
    def convert_tokens_to_ids(self, tokens):
        """
        Converts a single token, or a sequence of tokens, (str) in a single
        integer id (resp.
    ...
    def encode(
        ...
        **kwargs
        ...
        """
        Converts a string in a sequence of ids (integer), using the tokenizer
        and vocabulary.
        ...
        Args:
            ...
            pad_to_max_length: if set to True, the returned sequences will be padded according to the models's padding side and
                padding index, up to their max length. If no max length is specified, the padding is done up to the models's max length.
    ...
    def encode_plus(
        ...
        **kwargs
        ...
        and additional informations: the mask for sequence classification and
        ...
        Return:
            ...
            With the fields:
                ``input_ids``: list of token ids to be fed to a models
                ``token_type_ids``: list of token type ids to be fed to a models
                ``attention_mask``: list of indices specifying which tokens should be attended to by the models
    ...
    def batch_encode_plus(
        ...
        **kwargs
        ...
        if return_attention_masks:
            # Allow the models to not give any special attention to padded input
            batch_outputs["attention_mask"] = [
                [0] * len(v) for v in batch_outputs["input_ids"]
            ]
        ...
        if return_tensors is not None:
            ...
            for key, value in batch_outputs.items():
                ...
                if key != "input_len" and self._pad_token is not None:
                    ...
                    padded_value = [
                        v
                        + [self.pad_token_id if key == "input_ids" else 1]
                        * (max_seq_len - len(v))
        ...
        if return_attention_masks:
            if is_tf_available():
                batch_outputs["attention_mask"] = tf.abs(
                    batch_outputs["attention_mask"] - 1
                )
            else:
                batch_outputs["attention_mask"] = torch.abs(
                    batch_outputs["attention_mask"] - 1
                )
    ...
    def prepare_for_model(
        ...
        so that it can be used by the models. It adds special tokens, truncates
        sequences if overflowing while taking into account the special tokens
        and manages a window stride for overflowing tokens.
        ...
        if (
            ...
            logger.warning(
                ...
                "for this models ({} > {}). Running this sequence through the models will result in "
        ...
        if pad_to_max_length and max_length is None and self.max_len > 10000:
            logger.warning(
                "Sequence can't be padded as no maximum length is specified and the models maximum length is too high."
            )

        if needs_to_be_padded:
            difference = (
                max_length if max_length is not None else self.max_len
    ...
    def truncate_sequences(
        ...
        if truncation_strategy == "longest_first":
            ...
            assert (
                pair_ids is not None and len(pair_ids) > num_tokens_to_remove
            )
    ...
    def build_inputs_with_special_tokens(self, token_ids_0, token_ids_1=None):
        ...
        A RoBERTa sequence has the following format:     single
        sequence: <s> X </s>     pair of sequences: <s> A </s></s> B
        </s>
        """
        if token_ids_1 is None:
            return token_ids_0
    ...
    def convert_ids_to_tokens(self, ids, skip_special_tokens=False):
        ...
        " (resp.) a sequence of tokens (str), using the vocabulary and added
        tokens.
    ...
    def convert_tokens_to_string(self, tokens):
        ...
        '.join(self.convert_ids_to_tokens(token_ids)) but we often want
    ...
    def decode(
        ...
        """
        Converts a sequence of ids (integer) in a string, using the tokenizer
        and vocabulary with options to remove special tokens and clean up
        tokenization spaces. Similar to doing ``self.convert_tokens_to_string(s
        elf.convert_ids_to_tokens(token_ids))``.
    ...
    def special_tokens_map(self):
        """
        A dictionary mapping special token class attribute (cls_token,
        unk_token...) to their values ('<unk>', '<cls>'...)
        """
        set_attr = {}
...
class PreTrainedTokenizerFast(PreTrainedTokenizer):
    ...
    def _convert_encoding(
        ...
        if return_overflowing_tokens:
            ...
            encoding_dict["overflowing_tokens"] = (
                overflowing.ids if overflowing is not None else []
            )
        if return_special_tokens_mask:
        ...
        # Prepare inputs as tensors if asked
        if return_tensors == "tf" and is_tf_available():
            encoding_dict["input_ids"] = tf.constant(
                [encoding_dict["input_ids"]]
            )
            ...
            encoding_dict["input_ids"] = torch.tensor(
                [encoding_dict["input_ids"]]
            )
            if "token_type_ids" in encoding_dict:
    ...
    def decode_batch(
        ...
        ids_batch,
        skip_special_tokens=False,
        clear_up_tokenization_spaces=True,
    ):
        return [
            self.clean_up_tokenization(text)
            if clear_up_tokenization_spaces
            else text
            for text in self.tokenizer.decode_batch(
                ids_batch, skip_special_tokens
            )
        ]
